...
impl Almanac {
    pub fn translate_seeds_min(&self) -> GardenItem {
        self.seeds
            .iter()
            .map(|seed_range| {
                println!("Onto a seed range: {:?}", seed_range);
                let mut min = GardenItem::Location(usize::MAX);
                for seed in seed_range.start..seed_range.end {
                    min = cmp::min(min, self.translate_seed(seed));
                }
                min
            })
            .min()
...
fn main() {
    ...
    println!("{:#?}", almanac);
    let min_loc = almanac.translate_seeds_min();
