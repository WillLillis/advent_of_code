...
fn set_planned_pos(elves: &mut Vec<Elf>, curr_elves: &HashSet<XY>, first_check_idx: usize) -> u32 {
    ...
    for elf in elves.iter_mut() {
        match get_next_move(elf, curr_elves, first_check_idx) {
            Some(pos) => {
                if !plan_elves.insert(pos) {
                    collisions.insert(pos);
                    elf.plan_pos = None; // unecessary, will mark as None later
                } else {
                    elf.plan_pos = Some(pos);
                }
            },
            None => {
                elf.plan_pos = None;
...
fn main() {
    let mut elves = get_elves("test_input.txt");
