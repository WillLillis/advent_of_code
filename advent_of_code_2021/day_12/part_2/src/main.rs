...
fn valid_visit(
    ...
    if cave_info[next_cave_idx].name == "start" {
        return (false, false);
    }
    // must be reachable
    ...
    if cave_info[next_cave_idx].size == CaveSize::Large {
        ...
        return (true, false);
    } else if cave_info[next_cave_idx].size == CaveSize::Small && !twice_small {
        return (true, true);
    } else {
        return (false, false);
    }
    ...
    for j in 0..adj_matrix.len() {
        match valid_visit(
            ...
            (true, false) => {
                ...
                // try the visit...
                visited[j] = true;
                n_paths += visit_valid_paths(j, visited, adj_matrix, cave_info, twice_small);
                // ...and backtrack
                visited[j] = false;
            }
            (true, true) => {
                // second visit to a small cave
                n_paths += visit_valid_paths(j, visited, adj_matrix, cave_info, true);
            }
            _ => {}
        }
    }
...
fn main() {
    ...
    let n_paths = visit_valid_paths(start_idx, &mut visited, &adj_matrix, &cave_info, false);
