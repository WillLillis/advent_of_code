...
impl MonkeyOp {
    fn new(op_char: char, op_num: Option<i32>) -> Self {
        match op_num {
            Some(num) => {
                match op_char {
...
impl Monkey {
    pub fn new(op: MonkeyOp, items: VecDeque<i32>, test_num: i32, true_dest: usize,
...
pub fn monkey_file_to_vec() -> Vec<Monkey> {
    ...
    loop {
        ...
        let starting_items: VecDeque<i32> = starting_items
            .split(&[':', ','][..])
            .into_iter()
        ...
        let op: Vec<&str> = op
            .split_inclusive(&['+', '*'][..])
            .collect();
        let op_num: Option<i32> = match op.last().unwrap().trim().parse::<i32>() {
            Ok(x) => Some(x),
            _ => None,
        };
        let op = MonkeyOp::new(
...
fn main() {
    ...
    let monkey_business = inspections.iter().take(2).product::<u32>();
