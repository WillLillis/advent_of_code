...
fn main() {
    ...
    let key: usize;
    let mut div_1_idx: usize = 0;
    let mut div_2_idx: usize = 0;
    let packet_div_1 = List {items: vec![Element::Integer(2)]};
    let packet_div_2 = List {items: vec![Element::Integer(6)]};

    packets.push(packet_div_1.clone());
    packets.push(packet_div_2.clone());
    ...
    loop {
        ...
        packets.push(left_parsed);
        packets.push(right_parsed);
    }

    packets.sort();

    for (idx, packet) in packets.iter().enumerate() {
        if packet == &packet_div_1 {
            div_1_idx = idx + 1; 
        } else if packet == &packet_div_2 {
            div_2_idx = idx + 1;
        }
    }

    key = div_1_idx * div_2_idx;

    println!("Distress signal decoder key: {key}");    
}
