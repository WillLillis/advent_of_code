...
public: 
    ...
    static size_t solution(Dir* dir_in) {
        size_t accum = 0;
        for(int i = 0; i < dir_in->sub_dirs.size(); i++) {
            accum += solution(dir_in->sub_dirs[i]);
        }
...
int main(void) {
    ...
    // now we need to sum up all dir sizes that are <= 100,000
    std::cout << FileSystem::solution(&root_dir) << std::endl;
