...
pub fn naive(i: &str) -> usize {
    // same as before
    let i = fs::read_to_string(i).unwrap();
    let jets = i
        .trim()
        .chars()
    ...
    while total_rocks < 1000000000000 {
        ...
        if !cycle_found {
            ...
            if let Some(state_data) = states.get(&state) {
                ...
                println!("Cycle height: {}, cycle len: {}", height_gain_in_cycle, rocks_in_cycle);
                println!("Rock #: {}", total_rocks - (skipped_cycles * rocks_in_cycle));
...
fn main() {
    println!("Highest: {}", naive("test_input.txt"));
}
